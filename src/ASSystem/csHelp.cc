...
bool csHelp::moduleTextHelp( std::string moduleNameIn, std::string versionString ) 
{
  ...
  std::vector<std::string> names = csMethodRetriever::getStandardModuleNames();
  csParamDef pdef;
  ...
  try {
    ...
    if( !moduleName.compare(",") ) {
      for( int i = 0; i < numModules; i++ ) {
        pdef.clear();
        if( versionString.length() == 0 ) {
          csMethodRetriever::getParamMethod( names.at(i) )( &pdef );
        }
        else {
          csMethodRetriever::getParamMethod( names.at(i), versionString )( &pdef );
        }
        moduleTextHelpFull( pdef );
      }
      return true;
      ...
      for( int i = 0; i < numModules; i++ ) {
        if( (int)(names.at(i).length()) >= length ) {
          if( !moduleName.compare( names.at(i).substr(0,length) ) ) {
            pdef.clear();
            if( versionString.length() == 0 ) {
              csMethodRetriever::getParamMethod( names.at(i) )( &pdef );
            }
